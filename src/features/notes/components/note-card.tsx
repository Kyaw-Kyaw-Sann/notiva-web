...
export function NoteCard({ note }: { note: Note }) {
  return (
    <article className={cn("min-h-56 rounded-xl border shadow-card transition-transform hover:-translate-y-0.5", noteBackgroundClasses[note.backgroundColor])}>
      <Link href={`/notes/${note.id}`} className="flex min-h-56 flex-col rounded-xl p-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={`Open ${note.title}`}>

...
export function AiConversationsPage() {
  ...
  return (
    <section className="mx-auto flex h-[calc(100dvh-8rem)] min-h-0 w-full max-w-7xl flex-col sm:min-h-[38rem]">
      ...
      {view === "search" ? <div className="notiva-scrollbar flex-1 overflow-y-auto pb-6"><SemanticSearch /></div> : (
        <div className="grid min-h-0 flex-1 overflow-hidden rounded-2xl border bg-background shadow-card md:grid-cols-[17rem_minmax(0,1fr)]">
          <div className={cn("min-h-0", mobileChatOpen && "hidden md:block")}>
            {conversations.isLoading ? <div className="flex h-full items-center justify-center border-r"><LoaderCircle className="size-6 animate-spin text-primary" aria-label="Loading conversations" /></div> : conversations.isError ? <div className="h-full border-r p-4 text-sm" role="alert"><p>{normalizeApiError(conversations.error).message}</p><Button type="button" variant="outline" size="sm" className="mt-3" onClick={() => void conversations.refetch()}>Try again</Button></div> : <ConversationList conversations={conversations.data ?? []} createError={createConversation.isError ? normalizeApiError(createConversation.error).message : undefined} creating={createConversation.isPending} onCreate={() => void create()} onSelect={(conversationId) => { setSelectedId(conversationId); setMobileChatOpen(true); }} selectedId={activeId} />}
          </div>
          <div className={cn("min-h-0", !mobileChatOpen && "hidden md:block")}>
            {activeConversation ? <AiChatPanel conversation={activeConversation} onBack={() => setMobileChatOpen(false)} onDeleted={() => { setSelectedId(null); setMobileChatOpen(false); }} /> : <div className="flex h-full min-h-80 flex-col items-center justify-center px-6 text-center"><MessageCircle className="size-9 text-primary" /><h2 className="mt-4 font-semibold">Chat across all notes</h2><p className="mt-1 max-w-sm text-sm text-muted-foreground">Create an all-notes conversation to ask questions using your Notiva knowledge.</p><Button type="button" className="mt-5" disabled={createConversation.isPending} onClick={() => void create()}>{createConversation.isPending && <LoaderCircle className="animate-spin" />}New conversation</Button>{createConversation.isError && <p className="mt-3 text-xs text-destructive" role="alert">{normalizeApiError(createConversation.error).message}</p>}</div>}
          </div>
        </div>
      )}
    </section>
